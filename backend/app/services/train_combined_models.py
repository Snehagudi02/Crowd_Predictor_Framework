...
def load_data():
    ...
    try:
        if os.path.exists('storage/processed/processed_crowd_data.csv'):
            df_live = pd.read_csv('storage/processed/processed_crowd_data.csv')
            # Filter capped and zero values
            df_live = df_live[(df_live['count'] != 500.0) & (df_live['count'] > 0)]
            df_live['timestamp'] = pd.to_datetime(df_live['timestamp'], dayfirst=True)
        ...
        print("Could not parse storage/processed/processed_crowd_data.csv:", e)

    # 2. Historical Raw Dataset
    try:
        if os.path.exists('storage/raw/crowd_data.csv'):
            df1 = pd.read_csv('storage/raw/crowd_data.csv')
        ...
        print("Could not parse storage/raw/crowd_data.csv:", e)
        
    # 2. crowd_counts.csv (Processed tracker context)
    try:
        df2 = pd.read_csv('storage/processed/crowd_counts.csv', names=['timestamp', 'count'])
    ...
    try:
        df3 = pd.read_csv('storage/raw/pedestrian_data.csv')
    ...
    try:
        df4 = pd.read_csv('storage/raw/shanghaitech_data.csv')
