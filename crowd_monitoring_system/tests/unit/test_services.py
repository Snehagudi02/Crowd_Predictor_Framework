import pytest
import numpy as np
import cv2
from unittest.mock import patch, MagicMock
from backend.app.services.density import process_image
from backend.app.services.forecast import train_system, get_predictions, get_predictive_risk


# ──────────────────────────────────────────────
# Fixtures
# ──────────────────────────────────────────────

@pytest.fixture
def blank_image_bytes():
    """
    Encodes a blank 100×100 black frame as JPEG bytes.
    Used by density tests — no real image/dataset is touched.
    """
    frame = np.zeros((100, 100, 3), dtype=np.uint8)
    _, buffer = cv2.imencode(".jpg", frame)
    return buffer.tobytes()


# ──────────────────────────────────────────────
# Density service tests
# ──────────────────────────────────────────────

def test_process_image_valid(blank_image_bytes):
    with patch("backend.app.services.density.cv_pipeline") as mock_cv:
        mock_cv.process_frame.return_value = {"count": 10}
        result = process_image(blank_image_bytes)
        assert result["count"] == 10


def test_process_image_invalid():
    with pytest.raises(ValueError, match="Invalid image"):
        process_image(b"not an image")


# ──────────────────────────────────────────────
# Forecast service tests
# (All calls are mocked — ml_pipeline never touches the real dataset)
# ──────────────────────────────────────────────

def test_train_system_service():
    with patch("backend.app.services.forecast.ml_pipeline") as mock_ml:
        mock_ml.train_model.return_value = {"status": "success"}
        assert train_system() == {"status": "success"}


def test_get_predictions_service():
    with patch("backend.app.services.forecast.ml_pipeline") as mock_ml:
        mock_ml.get_forecast.return_value = [{"y": 1}]
        assert get_predictions(10) == [{"y": 1}]


def test_get_predictive_risk_service():
    with patch("backend.app.services.forecast.ml_pipeline") as mock_ml:
        # Case 1: empty forecast → risk should be "Unknown"
        mock_ml.get_forecast.return_value = []
        result = get_predictive_risk()
        assert result["risk"] == "Unknown"

        # Case 2: forecast present → delegate to predict_future_risk
        mock_ml.get_forecast.return_value = [{"predicted_count": 10}]
        with patch("backend.app.services.forecast.predict_future_risk") as mock_risk:
            mock_risk.return_value = {"first_high_risk_time": None, "peak_prediction": 10}
            result = get_predictive_risk()
            assert result["peak_prediction"] == 10
