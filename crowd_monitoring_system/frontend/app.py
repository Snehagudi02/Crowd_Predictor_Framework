...
import pandas as pd
from frontend.api import upload_frame, get_forecast, train_model
...
def render_graphs(histories):
    ...
    with graph_ph.container():
        ...
        for i in range(4):
            ...
            if df_forecast.empty or 'predicted_count' not in df_forecast.columns:
                continue
...
if check_backend():
    status_ph.success("Backend: Online")
else:
    status_ph.warning("Backend: Waking up / Offline")
...
if input_source == "Upload Video":
    ...
    temp_dir = os.path.join(tempfile.gettempdir(), "crowd_temp")
            ...
            if caps[i] and caps[i].isOpened():
                ...
                if is_success:
                    ...
                    if res:
                        ...
                        cv2.putText(frame_rgb, "API Offline / Waking Up", (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
